...
class Vel_x_rear_left(Node):
    def __init__(self):
        super().__init__('vel_x_rear_left')
    ...
    def odom_callback(self, msg):
        ...
        vel_x_rl = linear_vel_x + 0.5 * angular_yaw * rear_track_dist
        
        processed = Float64()
        processed.data = vel_x_rl
        

        # Publish processed velocity
        self.publisher.publish(processed)
        self.get_logger().info(f"Published processed x velocity of left rear wheel: vel_x_rl={processed.data}")

def main(args=None):
    rclpy.init(args=args)
    node = Vel_x_rear_left()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
