...
class Vel_y_front_right(Node):
    def __init__(self):
        super().__init__('vel_y_front_right')
    ...
    def odom_callback(self, msg):
        ...
        long_dist = 1.7238
        vel_y_fr = linear_vel_y + angular_yaw * long_dist
        processed = Float64()
        processed.data = vel_y_fr
        

        # Publish processed velocity
        self.publisher.publish(processed)
        self.get_logger().info(f"Published processed y velocity of the front right wheel={processed.data}")

def main(args=None):
    rclpy.init(args=args)
    node = Vel_y_front_right()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
