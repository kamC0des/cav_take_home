...
class Vel_x_front_left(Node):
    def __init__(self):
        super().__init__('vel_x_front_left')
    ...
    def odom_callback(self, msg):
        ...
        vel_x_fl = linear_vel_x + 0.5 * angular_yaw * front_track_dist
        
        processed = Float64()
        processed.data = vel_x_fl
        

        # Publish processed velocity
        self.publisher.publish(processed)
        self.get_logger().info(f"Published processed x velocity of front right wheel: vel_x_fl={processed.data}")

def main(args=None):
    rclpy.init(args=args)
    node = Vel_x_front_left()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
