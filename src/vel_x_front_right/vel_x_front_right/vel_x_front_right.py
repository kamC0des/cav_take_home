...
class Vel_x_front_right(Node):
    def __init__(self):
        super().__init__('vel_x_front_right')
    ...
    def odom_callback(self, msg):
        ...
        front_track_dist = 1.638
        vel_x_fr = linear_vel_x - 0.5 * angular_yaw * front_track_dist
        
        processed = Float64()
        processed.data = vel_x_fr
        

        # Publish processed velocity
        self.publisher.publish(processed)
        self.get_logger().info(f"Published processed x velocity of front right wheel: vel_x_fr={processed.data}")

def main(args=None):
    rclpy.init(args=args)
    node = Vel_x_front_right()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
